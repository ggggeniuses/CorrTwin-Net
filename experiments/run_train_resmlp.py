...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train ResMLP surrogate.")
...
def main() -> None:
    ...
    run_dir = create_run_dir("resmlp", results_root / "runs", args.run_name)
    ...
    loader = DataLoader(
        TensorDataset(torch.from_numpy(arrays["train"]), torch.from_numpy(labels["train"])),
    ...
    best_ckpt_path = ckpt_dir / "resmlp_best.pt"
    ...
    save_json_file(results_root / "metrics" / "resmlp_metrics.json", metrics)
    save_json_file(run_dir / "metrics.json", metrics)
    plot_loss(history, results_root / "figures" / "resmlp_loss.png")
    save_csv(results_root / "metrics" / "resmlp_history.csv", history)
